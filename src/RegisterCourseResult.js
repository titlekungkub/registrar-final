...
import './css/RegisterCourseResult.css';

class RegisterCourseResult extends Component {
    constructor(props){
        super(props);
        this.state = {
            courses : [],
    ...
    componentDidMount(){
        fetch('/courses/studentRegistered', {
            method: 'POST',
            headers: new Headers({
        ...
        fetch('/courses/calculateTotalCredit', {
            method: 'POST',
            headers: new Headers({
                'Content-Type': 'application/x-www-form-urlencoded'
            }),
    ...
    render() {
        return (
            <div className='RegisterCourseResultPage'>
                <p className='H1'>รายวิชาที่แสดงความจำนง</p>
                <table>
                    <tbody>
                        <tr key={1}>
...
export default RegisterCourseResult;